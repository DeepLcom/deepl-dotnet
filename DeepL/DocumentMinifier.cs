  ...
  public class DocumentMinifier : IDocumentMinifier {
    ...
    public string MinifyDocument(string inputFilePath, bool cleanup = false) {
      ...
      try {
        ...
        throw new DocumentMinificationException($"Exception when extracting document: " +
                                                $"Failed to extract {inputFilePath} to {extractedDocDirectory}", ex);
      }
      ...
      if (fileSizeResponse > MinifiedDocSizeLimitWarning) {
        throw new DocumentMinificationException(
            ...
            + "This might cause the translation to fail.", new WarningException());
      }
    ...
    public void DeminifyDocument(string inputFilePath, string outputFilePath, bool cleanup = false) {
      ...
      if (!Directory.Exists(extractedDocDirectory)) {
        try {
          ...
          throw new DocumentDeminificationException(
                $"Exception when deminifying, could not create directory at {extractedDocDirectory}.", ex
          );
        }
      }
      ...
      try {
        ...
        throw new DocumentDeminificationException($"Exception when extracting document: " +
                                                $"Failed to extract {inputFilePath} to {extractedDocDirectory}", ex);
      }

      ReplaceImagesInDir(extractedDocDirectory, mediaDir);
      try {
    ...
    ///   to the <c>mediaDirectory</c> and replace the media in the <c>inputDirectory</c> with a placeholder image. The
    ...
    private void ExportMediaToMediaDirAndReplace(string inputDirectory, string mediaDirectory) {
      using var placeholder = new MagickImage(MagickColors.Gray, 1, 1);
      foreach (var filePath in Directory.GetFiles(inputDirectory, "*.*", SearchOption.AllDirectories)) {
        if (SupportedMediaFormats.Contains(Path.GetExtension(filePath).ToLowerInvariant())) {
          ...
          try {
            ...
            placeholder.Write(filePath, MagickFormat.Png);
          } catch (Exception ex) {
    ...
    /// </param>
    /// <exception cref="DocumentMinificationException">
    ///   If a problem occurred when trying to reinsert the images
    /// </exception>
    private void ReplaceImagesInDir(string inputDirectory, string mediaDirectory) {
      foreach (var filePath in Directory.GetFiles(mediaDirectory, "*.*", SearchOption.AllDirectories)) {
        ...
        if (!string.IsNullOrWhiteSpace(curMediaDir) && !Directory.Exists(curMediaDir)) {
          try {
            ...
            throw new DocumentDeminificationException("Exception when reinserting images. " +
                                                      $"Failed to create directory at {curMediaDir}.", ex);
          }
        }
        ...
        try {
          ...
          throw new DocumentDeminificationException("Exception when reinserting images. " +
                                                    $"Failed to move media back to {curMediaPath}.", ex);
        }
      }
    }
  }
