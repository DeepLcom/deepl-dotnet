...
- `Minification`: A `bool` value. If set to `true`, the library will try to minify a document before translating it
-  through the API, sending a smaller document if the file contains a lot of media. This is currently only supported
-  for `pptx` and `docx` files. See also [Document minification](#document-minification). Note that this only works
-  in the high-level `TranslateDocumentDownloadAsync` method, not `TranslateDocumentUploadAsync`.
